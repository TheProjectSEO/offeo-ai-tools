...
import { Zap, Sparkles, Download, Copy } from 'lucide-react'
import Header from '@/components/Header'
import Footer from '@/components/Footer'

export default function SloganGenerationPage() {
  const [prompt, setPrompt] = useState('')
  const [isGenerating, setIsGenerating] = useState(false)
  const [generatedSlogans, setGeneratedSlogans] = useState<string[] | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [provider, setProvider] = useState<string | null>(null)

  const handleGenerate = async () => {
    if (!prompt.trim()) {
      setError('Please enter your business or product description')
      return
    }
  ...
  const handleCopyAll = () => {
    if (generatedSlogans) {
      navigator.clipboard.writeText(generatedSlogans.join('\n'))
    }
  }
      ...
      <section className="py-12 md:py-24">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-6" style={{background: 'var(--accent-warm)'}}>
            ...
            <span className="text-sm font-medium" style={{color: 'var(--text-primary)'}}>AI Slogan Generator</span>
          </div>
          ...
          <p className="text-lg md:text-xl max-w-2xl mx-auto mb-12" style={{color: '#666666'}}>
            Create catchy, memorable slogans for your business or product. Generate powerful taglines that capture your brand's essence in seconds.
          </p>
        </div>
      </section>
              ...
              <div>
                ...
                <div className="relative mb-6">
                  <textarea
                    ...
                    placeholder="A sustainable coffee shop that sources beans directly from farmers..."
                  ...
                  {generatedSlogans ? (
                    <div className="w-full space-y-4">
                      {generatedSlogans.map((slogan, index) => (
                        <div key={index} className="group flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                          <span className="text-sm text-gray-900 font-medium flex-1">{slogan}</span>
                          <button
                            onClick={() => handleCopySlogan(slogan)}
                            className="ml-3 p-1 opacity-0 group-hover:opacity-100 transition-opacity"
                            title="Copy slogan"
                          >
                            <Copy className="w-4 h-4 text-gray-500" />
                          </button>
                    ...
                    <div className="w-full text-center flex flex-col items-center justify-center h-full">
                      ...
                      <p className="text-gray-500">Your generated slogans will appear here</p>
                    </div>
                  )}
                </div>
                
                {/* Provider info below the slogans */}
                {generatedSlogans && provider && (
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-medium tracking-tight mb-4" style={{color: 'var(--text-primary)'}}>
              Create Memorable Brand Messages
            </h2>
            <p className="text-lg max-w-2xl mx-auto" style={{color: '#666666'}}>
              Our AI slogan generator creates catchy, memorable taglines that capture your brand's essence. Perfect for businesses, products, and marketing campaigns.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div className="text-center p-6">
              ...
              <p className="text-sm" style={{color: '#666666'}}>Advanced AI for catchy, memorable slogans</p>
            </div>
            ...
            <div className="text-center p-6">
              ...
              <h3 className="text-xl font-semibold mb-2" style={{color: 'var(--text-primary)'}}>Multiple Options</h3>
              <p className="text-sm" style={{color: '#666666'}}>Generate multiple slogans to choose from</p>
            </div>
            ...
            <div className="text-center p-6">
              ...
              <h3 className="text-xl font-semibold mb-2" style={{color: 'var(--text-primary)'}}>Brand Ready</h3>
              <p className="text-sm" style={{color: '#666666'}}>Perfect for marketing and branding</p>
            </div>
          </div>
        </div>
      </section>
