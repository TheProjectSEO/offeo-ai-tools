...
export async function POST(request: NextRequest) {
  try {
    ...
    // Add headshot-specific context to the prompt
    const headshotPrompt = `Professional headshot portrait: ${cleanPrompt}`
    safeLog('generate-headshot prompt:', truncate(cleanPrompt))
    const result = await generateImage(headshotPrompt, 'headshot')
