...
export async function POST(request: NextRequest) {
  try {
    ...
    // Add cartoon-specific context to the prompt
    const cartoonPrompt = `Cartoon style illustration: ${cleanPrompt}`
    safeLog('generate-cartoon prompt:', truncate(cleanPrompt))
    const result = await generateImage(cartoonPrompt, 'cartoon')
